...
int main()
{
    ...
    cout << "\n=== DEFENCE - MANUAL OPPONENT TEST ===";
    cout << "\nUse WASD to drive the opponent (attacker).";
    cout << "\nState machine: SEARCH -> FLEE / STRAFE / HIDE";
    ...
    // mode 0: single player with manual opponent (WASD)
    ...
    // mode 0 does not need join_player -- opponent is local
    ...
    while (1) {
        ...
        // --- my robot state from simulator (mode 0 = index 1) ---
        ...
        if (found) {
            for (int k = 0; k < S1->N_obs; k++) {
                ...
                // obstacle (within HIDE_NEAR_R). otherwise we might
                // be on the far side of the arena and an obstacle
                // just happens to be on the line -- that's not cover
        ...
        set_opponent_inputs(pw_l_o, pw_r_o, pw_laser_o, laser_o, 100.0);
