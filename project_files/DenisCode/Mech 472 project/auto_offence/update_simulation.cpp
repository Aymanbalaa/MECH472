...
int update_obstacles()
{
	...
	static double x_obs_static[1000], y_obs_static[1000];

	if (!init) {

		// temporary random positions (we refine later)
		x_obs_static[0] = 150 + rand() % 340;
		y_obs_static[0] = 80 + rand() % 320;

		x_obs_static[1] = 150 + rand() % 340;
		y_obs_static[1] = 80 + rand() % 320;
	...
	for (k = 0; k < N_obs; k++) {
		S1->x_obs[k] = x_obs_static[k];
		S1->y_obs[k] = y_obs_static[k];
	}
