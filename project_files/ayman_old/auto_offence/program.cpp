...
// Player 1 = offence (hosts simulation, shows image)
...
// -----------------------------------------------------------------------
// mask_obstacles -- paint over known obstacle footprints with bright gray
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// find_opponent -- vision pipeline: detect opponent position and heading
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// draw_circle_rgb -- outline circle
// -----------------------------------------------------------------------
void draw_circle_rgb(image &img, int cx, int cy, int r,
                     int R, int G, int B)
{
...
void drive_forward(int &pw_l, int &pw_r, double heading_err, int v_fwd, double steer_gain)
{
...
int main()
{
    ...
    const int N_obs = 2;
    double x_obs[N_obs] = { 0, 0 }; //     double x_obs[N_obs] = { 270.5, 135.0 };
    ...
    // --- Laser state (one shot only) ---
    int fired = 0;

    // --- State machine: 0=SEARCH  1=CHASE  2=FIRE  3=EVADE ---
    int state = 0;
    double evade_start = -999.0;
    static const char *snames[] = {"SEARCH", "CHASE", "FIRE", "EVADE"};
    ...
    const int    WALL_MARGIN = 20;    // wall avoidance margin
    const int    LOST_THRESH =  25;    // frames before SEARCH
    const double EVADE_DUR   =  2.0;   // evasion duration (seconds)
    const double AIM_TOL     = 0.25;   // opponent heading tolerance for evade trigger
    const double AIM_DIST    = 280.0;  // evade trigger distance
    const double BODY_R      =  60.0;  // half robot body length

    cout << "\n=== AUTO OFFENCE (Player 1) ===";
    cout << "\nState machine: SEARCH -> CHASE -> FIRE -> EVADE";
    ...
    // Player 1: set up everything, then wait last (matches professor's example order)
    set_simulation_mode(1);
    set_robot_position(500, 240, PI);

    set_inputs(pw_l, pw_r, pw_laser, laser, max_speed);

    // --- Allocate vision images ---
    ...
    while (1) {
        ...
        // --- Obstacle check: find closest obstacle ---
        bool near_obs = false;
        ...
        // --- Defence trigger: is opponent aiming at me? ---
        ...
        // --- Line-of-sight check: is the path to the opponent clear of obstacles? ---
        ...
        if (found) {
            for (int k = 0; k < S1->N_obs; k++) {
                ...
                if (perp < 70.0) { clear_shot = false; break; }  // obstacle blocks the line
            }
        }

        // --- State transitions (only one per frame) ---
        if (opp_aiming && state != 3) {
            state = 3; evade_start = tc;
        } else if (lost_frames > LOST_THRESH && state != 3) {
            state = 0;
            ...
            state = 2;  // CHASE -> FIRE: clear shot from range OR close enough
        } else if (state == 2 && fabs(aim_err) > ALIGN_TOL * 2.5) {
            state = 1;  // lost aim -> back to CHASE
        } else if (state == 3 && (tc - evade_start) > EVADE_DUR) {
            state = found ? 1 : 0;
        }
        ...
        if (near_wall) {
            ...
            // Drive AROUND the obstacle, not away from it
            ...
            avoid_frames--;
            // Steer 90 degrees to the chosen side of "away from obstacle"
            double away_a = atan2(my_y - S1->y_obs[obs_closest_k],
                                  my_x - S1->x_obs[obs_closest_k]);
            double side_a = away_a + avoid_side * PI / 3.0;  // 60°: around + away
            double err_side = angle_diff(theta, side_a);
            drive_forward(pw_l, pw_r, err_side, V_FWD, STEER_GAIN);

        } else {
            // Emergency separation: if vision blind and too close, use simulator positions
            double rr_dx = my_x - S1->P[2]->x[2];
            double rr_dy = my_y - S1->P[2]->x[3];
            ...
            if (!found && sim_rr_dist < BACKUP_DIST) {
                ...
                case 3: // EVADE: strafe perpendicular to threat line
                    {
                        double ang_to_me = atan2(my_y - opp_nav_jc, my_x - opp_nav_ic);
                        ...
                        if (fabs(ev) > 0.5) rotate_inplace(pw_l, pw_r, ev, ROT_PW);
        ...
        // --- Fire laser: ONE SHOT then freeze ---
        if (!fired && tc > 3.0 && (state == 1 || state == 2) &&
            fabs(aim_err) < ALIGN_TOL && clear_shot && dist < FIRE_DIST)
        {
            fired = 1;
            laser = 1; pw_laser = 1500;
            pw_l = 1500; pw_r = 1500;  // stop motors

            // Draw final overlays on the freeze frame
            {
            ...
            for (int k = 0; k < S1->N_obs; k++) {
                //draw_circle_rgb(img_rgb, (int)S1->x_obs[k], (int)S1->y_obs[k],
                //                (int)OBS_AVOID, 255, 80, 80);
            }
